...
public class RunNetworkParameters {
	...
	public static void main(String[] args) throws FileNotFoundException {
		 ...
		 * 
		 * For serious applications, it is strongly recommended to use an initial plans
		 * file that results from a converged simulation with the initial decision
		 * variables.
		 */
		 ...
		 * 
		 * This requires to set (i) the number of iterations until the simulation has
		 * converged and (ii) the number of iterations over which to average to get rid
		 * of the simulation noise.
		 * 
		 * (i) The number of iterations until the simulation has converged is relative
		 * to the amount of variability in the decision variable randomization. Let X be
		 * any decision variable and Y be a random variation thereof. Let the simulation
		 * start with a converged plans file obtained with decision variable X. The
		 * number of iterations must then be large enough to reach a new converged state
		 * for any decision variable Y.
		 * 
		 * (ii) The number of iterations over which to average should be large enough to
		 * make the remaining simulation noise small compared to the expected difference
		 * between the objective function values of any decision variable and its random
		 * variation.
		 */
		 ...
		 * is defined here. The time grid should be small enough to capture all dynamics
		 * that are relevant to the problem at hand, but otherwise it should be chosen
		 * as coarse as possible to save computing resources.
		 */
		...
		modes.add("car");
		// TODO NEW 2018-09-24
		matsim.addSimulationStateAnalyzer(new DifferentiatedLinkOccupancyAnalyzer(timeDiscretization, modes,
				new LinkedHashSet<>(scenario.getNetwork().getLinks().keySet())));
		// matsim.addSimulationStateAnalyzer(new
		// DifferentiatedLinkOccupancyAnalyzer.Factory(timeDiscretization, modes,
		// new LinkedHashSet<>(scenario.getNetwork().getLinks().keySet())));

		/*
		 * Further parameters needed to run the optimization.
		 * 
		 * An initial decision variable (recall that the initial plans file should be
		 * converged given this decision variable) and the number of search iterations
		 * (improvement steps).
		 */
		...
		// changed it a bit.
		// final RandomSearch<NetworkParameters> randomSearch =
		// factories.newRandomSearch(matsim, randomizer,
		 ...
		 *
		 * opdyts.log gives very detailed statistics, also about the internals of the
		 * algorithm.
		 * 
		 */
